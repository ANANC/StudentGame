...
private:
	...
	XAudio2SoundPlayer *pBoomMusic;//爆炸音效
	XAudio2SoundPlayer *pClickMusic;//点击音效

public:
	void Init();//初始化
	BOOL MClick(POINT p);//点击
	void GetTouchNumber(GameWaterSmall *, int);//获得要检测接触的下标
	void WaterTouch();//接触
	void InitWprocession(int,int,int,int);//初始化水滴的状态
	...
	BOOL IsHaveSmallWaters();//是否存在小水滴
	void SetWatersVisible(bool);//设置全部水滴的可见性
};