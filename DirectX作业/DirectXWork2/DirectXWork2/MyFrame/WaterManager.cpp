...
{	
	for (int i = 0; i < row; i++)
	{
		for (int j = 0; j < col; j++)
		{
			...
			D2D1_POINT_2F p = { i*100 + 11, j*100 + 14 };
...
void GWaterManager::MClick(POINT p)
{
...
void GWaterManager::InitWprocession(int L,int M,int B)
{
	srand(time(NULL));
	for (int i = 0; i < L; i++)
	{
	...
	{
		int k = rand() % (col*row);
		pGameWater[k]->SetGWProgress(middle);
		pGameWater[k]->ChangeWaterProgress();
	}
	for (int i = 0; i < B; i++)
	{
