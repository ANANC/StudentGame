...
CGame::CGame(HINSTANCE hInstance,HWND hWnd,int nFps)
{
	...
	pInput = new CInput(hInstance);						//创建封装DirectInput的对象
...
HRESULT CGame::Init(HWND hWnd)
{
	...
	CNSpritesManager::GetInstance()->SetRenderTarget(pRenderTarget);
	return hr;
...
void CGame::Cleanup()
{
	...
	if (CNSpritesManager::GetInstance())
	{
		delete CNSpritesManager::GetInstance();
	}
	if(pInput)
	{
		delete pInput;
...
void CGame::Update(float fDeltaTime)
{
	...
	CNSpritesManager::GetInstance()->Update(fDeltaTime);					//帧函数，根据系统流逝的时间更新链表里的精灵，可以在子类重载，写游戏规则
