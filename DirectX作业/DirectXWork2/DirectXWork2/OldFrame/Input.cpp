...
CInput::CInput(HINSTANCE hInstance)
{
	bUseJoystick = false;
	pKeyboard = NULL;
	pMouse    = NULL;
...
bool CInput::CreateKeyboard(HWND hwnd)
{
	...
	if (FAILED(pKeyboard->SetCooperativeLevel(hwnd, DISCL_NONEXCLUSIVE | DISCL_BACKGROUND)))
...
{
	if (FAILED(pKeyboard->GetDeviceState(256, (LPVOID)keyBuffer)))
	{
		// re-acquire access to keyboard
		HRESULT hr = pMouse->GetDeviceState(sizeof(DIMOUSESTATE), &mouseBuffer);
...
bool CInput::CreateMouse(HWND hwnd)
{
	...
	if (FAILED(pMouse->SetCooperativeLevel(hwnd, DISCL_NONEXCLUSIVE | DISCL_BACKGROUND)))
	{
...
bool CInput::ReadMouse()
{
	...
	if (FAILED(pMouse->GetDeviceData(sizeof(DIDEVICEOBJECTDATA),
		mouseBuffer, &read_num, 0)))
	{
		pMouse->GetDeviceState(256, (LPVOID)keyBuffer);
