...
public:
	...
	int			GetnFps()  { return nFps; }				//得到动画帧频	

	

	void        SetTexturePos(int tx,int ty);			//设置从精灵图片上开始切割的位置
	POINT		GetTexturePos();						//得到精灵图片的切割坐标位置
	void		SetTexWidth(int tw){ nTexWidth = tw; }	//设置精灵纹理的宽度
	void		SetTexHeight(int th){ nTexHeight = th; }	//设置精灵纹理的高度
