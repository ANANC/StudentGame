...
public enum uiprefabName
{
start,newGame,select,set,about,exit,load,teach
};
...
public static class UIcommon  {
    ...
    public static bool IsMoveTorightpage = false;
    public static Color white = new Color(1, 1, 1, 1);
    public static string loadsceneName = "";
    
    public static GameObject GetPrefab(prefabType type,string name)
    {
        Object ob = Resources.Load(type.ToString() + "/" + name);
        return GameObject.Instantiate(ob) as GameObject;
    }

    public static void UIinit(Transform tra)
    {
        SetButtonAnimatorEnabled(tra,false, false);
        SetButtonAnimatorEnabled(tra,true, false);
    }

    public static void UIselectbutton(Transform tra,bool IsRight)
    {
        SetButtonAnimatorEnabled(tra, IsRight, true);
        SetButtonAnimatorEnabled(tra, !IsRight,false);
    }

    static void SetButtonAnimatorEnabled(Transform tra,bool IsRight,bool enabled)
    {
        string name = IsRight ? "right" : "left";
        SetGameObjectAnimator(tra.FindChild(name), enabled);
    }

    public static void UINotselectbutton(Transform tra, bool IsRight)
    {
        SetButtonAnimatorEnabled(tra, IsRight, false);
    }

    public static void SetGameObjectAnimator(Transform tra, bool enabled)
    {
        tra.GetComponent<Animator>().enabled = enabled;
        if (enabled == false)
            tra.GetComponent<SpriteRenderer>().color = white;
